...
function restoreActiveDownloads() {
  ...
  safeStorageGet(null, (items) => {
    ...
    originalConsoleLog(
      `📥 Found ${downloadKeys.length} potential downloads to restore`,
      {
        downloadIds: downloadKeys.map((k) =>
          k.replace("downloadProgress_", ""),
        ),
        currentVideoId,
        progressValues: downloadKeys.map((k) => ({
          id: k.replace("downloadProgress_", ""),
          progress: items[k],
        })),
      },
    );
      ...
      if (isInProgress) {
        ...
        if (storedInfo) {
          try {
            ...
            originalConsoleLog(
              "📦 Found download info in storage, attempting immediate restore:",
              downloadId,
            );
            ...
            originalConsoleLog(
              "✅ Restoring download notification immediately from storage:",
              downloadId,
              {
                videoId: info.videoId,
                currentVideoId,
                note:
                  info.videoId !== currentVideoId
                    ? "(different video, but showing anyway)"
                    : "(same video)",
              },
            );
            try {
              ...
              originalConsoleLog(
                "✅ Successfully restored immediately:",
                downloadId,
              );
              return; // Don't wait for background script response
            } catch (error) {
              originalConsoleError(
                "❌ Error in immediate restore, will try background script:",
                downloadId,
                error,
              );
            ...
            originalConsoleError(
              "⚠️ Failed to parse stored info, will try background script:",
              downloadId,
              e,
            );
            // Fall through to background script check
        ...
        safeSendMessage(
          ...
          (response) => {
            ...
            if (response && response.info) {
              ...
              // Users should see all their active downloads even when navigating between videos
              originalConsoleLog(
                "✅ Restoring download notification:",
                downloadId,
                {
                  progress,
                  status,
              ...
              try {
                ...
                originalConsoleLog(
                  "✅ Successfully restored download notification and started polling:",
                  downloadId,
                );
              } catch (error) {
                originalConsoleError(
                  "❌ Error restoring download notification:",
                  downloadId,
                  error,
                );
              }
              ...
              if (storedInfo) {
                try {
                  const info = JSON.parse(storedInfo);
                  console.log(
                    "Found download info in storage (background script not returned yet):",
                    downloadId,
                    info,
                  );
                  ...
                  // Restore ALL active downloads regardless of videoId
                  originalConsoleLog(
                    "✅ Restoring download notification from storage:",
                    downloadId,
                    {
                      progress,
                      status,
                  ...
                  try {
                    ...
                    originalConsoleLog(
                      "✅ Successfully restored download notification from storage:",
                      downloadId,
                    );
                  } catch (error) {
                    originalConsoleError(
                      "❌ Error restoring download notification from storage:",
                      downloadId,
                      error,
                    );
                  }
              ...
              console.log(
                "⚠️ Restoring download notification with minimal info (no downloadInfo found):",
                downloadId,
                "Progress:",
                progress,
                "Status:",
              ...
              try {
                ...
                console.log(
                  "✅ Successfully restored download notification with minimal info:",
                  downloadId,
                );
              } catch (error) {
                console.error(
                  "❌ Error restoring download notification with minimal info:",
                  downloadId,
                  error,
                );
              }
...
function extractVideoConfig(reason = "unknown") {
  ...
  try {
    ...
    scripts.forEach((script) => {
      ...
      if (
        content.includes("config_url") ||
        content.includes('"progressive"') ||
        content.includes('"hls"') ||
        content.includes('"qualities"')
      ) {
        try {
          ...
          if (jsonMatches) {
            jsonMatches.forEach((jsonStr) => {
              if (
                jsonStr.includes(".mp4") ||
                jsonStr.includes(".m3u8") ||
                jsonStr.includes("progressive") ||
                jsonStr.includes("hls")
              ) {
                try {
    ...
    const players = document.querySelectorAll(
      "[data-config-url], [data-player-id]",
    );
          ...
          .then((response) => {
            if (response.ok) {
              response
                ...
                .catch((e) =>
                  console.log("Failed to parse config from fetch:", e),
                );
            }
          })
...
function retryRestoreActiveDownloads() {
  if (
    restoreRetryCount < maxRestoreRetries &&
    isVideoPage() &&
    isExtensionContextValid()
  ) {
    restoreRetryCount++;
    console.log(
      `Retrying restore active downloads (attempt ${restoreRetryCount}/${maxRestoreRetries})`,
    );
    setTimeout(() => {
      restoreActiveDownloads();
      ...
      (response) => {
        ...
        if (
          ...
          if (retryCount < maxRetries) {
            ...
            console.log(
              `Button injection retry ${retryCount}/${maxRetries} (page loading: ${isPageLoading}, delay: ${delay}ms)`,
            );
          } else {
            console.log(
              "Button injection: Max retries reached, video data not available",
            );
          }
          return;
        }
...
function injectButtonElement(buttonContainer) {
  ...
  if (!document.getElementById("vimeo-downloader-button-styles")) {
    ...
    style.textContent = `
      ...
      .vimeo-downloader-download-btn {
        ...
        position: relative;
        font-family:i nherit !important;
      }
    ...
    safeSendMessage(
      ...
      (response) => {
        ...
        sortedQualities.forEach((video) => {
          ...
          const isHLS =
            video.type &&
            (video.type.includes("m3u8") || video.type.includes("hls"));
  ...
  downloadBtn.addEventListener("click", (e) => {
    ...
    if (
      ...
      console.warn(
        "Button URL belongs to different video, refreshing button data...",
        {
          buttonVideoId,
          currentVideoId,
...
function verifyButtonData() {
  ...
  if (currentVideoTitle && storedVideoTitle) {
    const normalizedCurrent = currentVideoTitle
      .toLowerCase()
      .trim()
      .replace(/\s+/g, " ");
    const normalizedStored = storedVideoTitle
      .toLowerCase()
      .trim()
      .replace(/\s+/g, " ");
...
const pageObserver = new MutationObserver(() => {
  ...
  if (
    !document.getElementById("vimeo-downloader-page-button-wrapper") &&
    !isRefreshing
  ) {
