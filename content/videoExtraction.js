  ...
  try {
    ...
    if (
      ...
      config.request.files.progressive.forEach((file) => {
        ...
        var q = typeof normalizeToDailymotionQuality === "function" ? normalizeToDailymotionQuality(file.quality || file.height) : (file.quality || file.height);
        sendUrlToBackground(file.url, q != null ? "mp4-" + q + "p" : "mp4-unknown");
      });
    }
...
function sendUrlToBackground(url, type) {
  ...
  if (
    urlVideoId &&
    currentPageVideoId &&
    String(urlVideoId) === String(currentPageVideoId)
  ) {
    ...
    (response) => {
      if (DEBUG_EXTRACTION)
        console.log(
          ...
          url,
          type,
          videoTitle || "(no title - different video)",
          urlVideoId || currentPageVideoId,
        );
    },
  );
}
              ...
              if (response.ok) {
                response
                  ...
                  .then((config) => {
                    if (DEBUG_EXTRACTION)
                      console.log(
                        "Fetched config from fetch intercept:",
                        config,
                      );
                    parseConfig(config);
                  })
