 ...
 * wait for video data (getVideoData with retries), avoid duplicates, then call injectButtonElement(container).
 * Called from content.js and after navigation/restore. Does not build DOM itself.
 */
function injectDownloadButton() {
  ...
  safeSendMessage(
    ...
    (response) => {
      ...
      if (document.getElementById(`ext-${videoId}`)) {
        return;
      }
      ...
      console.log(DM_BTN_LOG, "injectDownloadButton: injecting button");

      // Filter reliable URLs (same logic as popup/feed): exclude config, master, mp4-full; for HLS require quality
      ...
      const node = document.querySelector('[data-testid="like-button"]');
      ...
      const dropdownMenu = groupElem.querySelector(`#ext-dd-menu-${videoId}`);
      ...
      function startDownload(item) {
        ...
        const videoTitle = item.getAttribute("data-video-title") || safeTitle;
        const vid = item.getAttribute("data-video-id") || videoId;
        const convertToMp3 = item.getAttribute("data-convert-mp3") === "true";
        const baseName =
          (typeof sanitizeFilenameForDownload === "function"
            ...
            .trim()
            .slice(0, 200) || "video";
        const ext = convertToMp3
          ? "mp3"
          : type && type.includes("m3u8")
            ? "mp4"
            : "mp4";
