...
function injectButtonElement(buttonContainer) {
  ...
  const populateQualityMenu = (isRetry = false) => {
    ...
    safeSendMessage(
      ...
      (response) => {
        ...
        // Deduplicate by URL only: show all variants (multiple 1080p HLS = different bitrates)
        ...
        const deduplicatedQualities = Array.from(uniqueByUrl.values());
  ...
  // Append to action bar so Download sits in the same row as Like, Share, Bookmark
