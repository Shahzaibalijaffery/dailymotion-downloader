...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  // HLS (M3U8) download + FFmpeg (MP3/MP4) in content script + main world
  if (request.action === "downloadM3U8AndConvert") {
    if (window.self !== window.top) {
    ...
    (async () => {
      try {
        ...
        if (!url || typeof M3U8Downloader === "undefined") {
          sendResponse({
            success: false,
            error: url ? "M3U8Downloader not loaded" : "Missing URL",
          });
          return;
        }
        const downloader = new M3U8Downloader({ concurrency: 8, retries: 3 });
        let queueId;
        const cancelCheck = () =>
          new Promise((resolve) => {
            chrome.storage.local.get([`downloadCancelled_${downloadId}`], (r) =>
              resolve(!!r[`downloadCancelled_${downloadId}`]),
            );
          });
        ...
        const result = await downloader.download(
          ...
          async (progress) => {
            ...
            if ((await cancelCheck()) && queueId != null) {
              downloader.cancel(queueId);
            }
          },
          "video/mp4",
          true,
        ...
        const arrayBuffer =
          result instanceof ArrayBuffer
            ? result
            : await (await fetch(result)).arrayBuffer();
        if (typeof result === "string") URL.revokeObjectURL(result);

        const format = convertToMp3 ? "mp3" : "mp4";
        const outFilename = convertToMp3
          ? (filename || "audio").replace(/\.[^.]+$/, "") + ".mp3"
          : (filename || "video").replace(/\.[^.]+$/, "") + ".mp4";

        await chrome.storage.local.set({
          [`downloadStatus_${downloadId}`]: "Converting...",
        });

        window.postMessage(
          {
            type: "DAILYMOTION_FFMPEG_OPERATION",
...
window.addEventListener("message", (event) => {
  ...
  if (t === "DAILYMOTION_FFMPEG_RESULT") {
    ...
    const type =
      mimeType ||
      (name.endsWith(".mp3") ? "audio/mpeg" : "video/mp4");
