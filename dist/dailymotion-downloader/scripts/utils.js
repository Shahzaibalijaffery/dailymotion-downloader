...
/**
 * Check if a URL is a Dailymotion feed page (e.g. for-you reels)
 * Matches: dailymotion.com/pk, dailymotion.com/us#for-you, us.dailymotion.com, /following, etc.
 *
 * @param {string} url - The URL to check (optional, defaults to window.location.href in content)
 * @returns {boolean} - True if the URL is a feed/reels page
 */
...
// ---------------------------------------------------------------------------
// Quality utils – Dailymotion player uses these tiers; map all heights to them
// so labels match what the player shows (popup + page download button).
// ---------------------------------------------------------------------------

/** Dailymotion standard quality tiers (same as player settings) */
var DAILYMOTION_QUALITIES = [240, 380, 480, 720, 1080, 1440, 2160, 4320];

/**
 * Map any resolution height to standard quality tier (up to 8K).
 * E.g. 288, 360 → 380; 1352 → 1080; 2000 → 2160 (4K); 4000 → 4320 (8K).
 * @param {number|string} height - Height from manifest/type (e.g. 1352, 288)
 * @returns {number|null} - One of 240, 380, 480, 720, 1080, 1440, 2160, 4320 or null
 */
...
/**
 * Get display label for a quality number (e.g. "1080p").
 * Always returns a Dailymotion standard label so UI matches player.
 * @param {number|null} quality - Raw quality from type/URL (e.g. 1352, 720)
 * @returns {string} - "240p", "380p", "480p", "720p", "1080p", "1440p", "2160p", "4320p" or "Unknown Quality"
 */
...
/**
 * List of quality options for display (e.g. dropdowns).
 * @returns {number[]} - [240, 380, 480, 720, 1080, 1440, 2160, 4320]
 */
function getDailymotionQualityList() {
  return DAILYMOTION_QUALITIES.slice();
}

/**
 * Get short tag for a quality label (SD, HD, FHD, QHD, 4K, 8K, MP3).
 * Used for dropdown tags and button display.
 * @param {string} qualityLabel - e.g. "1080p", "720p", "480p", "MP3"
 * @returns {string|null} - "SD" | "HD" | "FHD" | "QHD" | "4K" | "8K" | "MP3" or null
 */
 ...
 * Creates user-friendly labels like "1080p (MP4)", "720p", etc. (no HLS suffix)
 *
