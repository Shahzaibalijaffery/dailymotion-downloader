...
/**
 * Fetch one M3U8 playlist (master or variant) and return segment count.
 * If master, fetches first variant to get segment count. Used to cap qualities at 720p when segments > 1500.
 * @param {string} m3u8Url - Playlist URL (master or variant)
 * @param {number|null} tabId - Tab ID for cookie/headers
 * @returns {Promise<{ segmentCount: number }>}
 */
async function getSegmentCountForM3u8Url(m3u8Url, tabId = null) {
  ...
  try {
    const url = typeof fixUrlEncoding === "function" ? fixUrlEncoding(m3u8Url) : m3u8Url;
    const fetchOptions = await getFetchOptionsWithHeaders(url, tabId, null);
    const response = await fetch(url, fetchOptions);
    if (!response.ok) return { segmentCount: 0 };
    const playlistText = await response.text();
    if (!playlistText || !playlistText.trim()) return { segmentCount: 0 };

    const { hasVideoVariants: isMaster } = getMasterPlaylistInfo(playlistText);
    let segments = [];

    if (isMaster) {
      const variantPlaylists = parseMasterPlaylist(playlistText, getBaseUrl(url));
      if (variantPlaylists.length === 0) return { segmentCount: 0 };
      let variantUrl = variantPlaylists[0].url;
      variantUrl = typeof fixUrlEncoding === "function" ? fixUrlEncoding(variantUrl) : variantUrl;
      const variantOptions = await getFetchOptionsWithHeaders(variantUrl, tabId, null);
      const variantResponse = await fetch(variantUrl, variantOptions);
      if (!variantResponse.ok) return { segmentCount: 0 };
      const variantText = await variantResponse.text();
      if (!variantText || !variantText.trim()) return { segmentCount: 0 };
      const parsed = parseM3U8(variantText, getBaseUrl(variantUrl));
      segments = parsed.segments || [];
    } else {
      const parsed = parseM3U8(playlistText, getBaseUrl(url));
      segments = parsed.segments || [];
    }
...
async function convertVideoToMp3AndDownload(
  ...
  const result = await new Promise((resolve) => {
    const timeoutId = setTimeout(() => {
      resolve({
        success: false,
        error: "MP3 conversion timed out. The helper may not support convertToMp3 or did not respond.",
      });
...
async function downloadAndMergeM3U8(
  ...
  try {
    ...
    const MAX_SIZE_FOR_MP4_CONVERT = 1.5 * 1024 * 1024 * 1024; // 1.5GB — above this, save as-is (no convert)
    ...
    if (!isLargeFile) {
      ...
      // Validate final blob size (property only, no read)
      ...
      // Read header BEFORE clearing source arrays — clearing finalBlobs/segmentBlobs first can
      // invalidate the blob's backing store in Chrome and cause NotReadableError when reading.
      const header = new Uint8Array(await mergedBlob.slice(0, 8).arrayBuffer());
      validatedBlob = mergedBlob;

      // Now safe to free segment/final arrays; we've materialized the read from mergedBlob.
      segmentData.length = 0;
      if (orderedSegments && orderedSegments.length) orderedSegments.length = 0;
      if (segmentBuffers && segmentBuffers.length) segmentBuffers.length = 0;
      segmentBlobs.length = 0;
    ...
    if (!alreadyMp4)
      try {
        const totalSizeForConvert = skippedMergeForLargeFile
          ? chunksOnlyForDownload.totalSize
          : validatedBlob.size;
        if (totalSizeForConvert > MAX_SIZE_FOR_MP4_CONVERT) {
          throw new Error(
            "File over 1.5 GB — saving as-is without conversion",
          );
        }
        await chrome.storage.local.set({
    ...
    if (!converted) {
      ...
      if (storedInputInIDB && inputBlobIdForConvert) {
        ...
        const savingExt = (finalFilename.match(/\.([^.]+)$/) || [null, "ts"])[1];
        await chrome.storage.local.set({
          [`downloadStatus_${downloadId}`]: `Saving as .${savingExt}...`,
        });
