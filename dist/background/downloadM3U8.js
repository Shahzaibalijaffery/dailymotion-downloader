 ...
 */
async function getFetchOptionsWithHeaders(url, tabId = null, abortController = null) {
  const options = {
    method: 'GET',
    headers: {
  ...
  if (tabId && tabId !== -1) {
    try {
      const cookies = await chrome.cookies.getAll({ url: 'https://www.dailymotion.com' });
...
function parseM3U8(playlistText, baseUrl) {
  ...
  const mapLines = lines.filter(line => {
    ...
    return upperLine.includes('#EXT-X-MAP') || 
           (upperLine.includes('MAP') && line.startsWith('#')) || 
           (upperLine.includes('INIT') && line.startsWith('#'));
  });
  if (mapLines.length > 0) {
    console.log(`Found ${mapLines.length} potential MAP/init line(s) in playlist`);
    ...
    if (upperLine.startsWith('#EXT-X-MAP')) {
      ...
      if (uriMatch1) {
        ...
        if (colonIndex !== -1) {
          ...
          if (afterColon && (afterColon.startsWith('http') || afterColon.startsWith('/') || afterColon.startsWith('./'))) {
            uri = afterColon;
      ...
      if (uri) {
        ...
        console.log('✅ Found #EXT-X-MAP with URI:', uri, '-> resolved to:', initSegmentUrl);
      } else {
        console.warn('⚠️ Found #EXT-X-MAP but could not extract URI. Full line:', line);
      }
  ...
  // Log summary
  if (initSegmentUrl) {
    console.log(`✅ parseM3U8: Found ${segments.length} segments and init segment: ${initSegmentUrl}`);
  } else {
    console.log(`⚠️ parseM3U8: Found ${segments.length} segments but NO init segment`);
    if (mapLines.length > 0) {
      console.warn(`  Note: Found ${mapLines.length} potential MAP line(s) but couldn't parse init segment URL`);
    }
  }
 ...
 */
async function downloadAndMergeM3U8(m3u8Url, filename, downloadId, abortController, tabId = null, downloadControllers, activeChromeDownloads, cleanupIndexedDBBlob, setupOffscreenDocument, blobToDataUrl) {
  try {
    ...
    try {
      const fetchOptions = await getFetchOptionsWithHeaders(m3u8Url, tabId, abortController);
      playlistResponse = await fetch(m3u8Url, fetchOptions);
      if (!playlistResponse.ok) {
        throw new Error(`Failed to fetch playlist: HTTP ${playlistResponse.status} ${playlistResponse.statusText}`);
      }
    } catch (error) {
      if (error.name === 'AbortError' || abortController.signal.aborted) {
        throw error;
      }
      throw new Error(`Network error: Could not fetch video playlist. ${error.message}`);
    }
    ...
    console.log(`Playlist size: ${playlistText.length} characters, ${playlistText.split('\n').length} lines`);
    ...
    const initialMapLines = initialLines.filter(line => {
      ...
      return upper.includes('MAP') || upper.includes('INIT') || upper.includes('EXT-X-MAP');
    });
    if (initialMapLines.length > 0) {
      console.log(`=== FOUND ${initialMapLines.length} MAP/INIT LINES IN INITIAL PLAYLIST ===`);
    ...
    if (isMasterPlaylist) {
      ...
      if (masterParsed.initSegmentUrl) {
        console.log('Found init segment in master playlist:', masterParsed.initSegmentUrl);
      ...
      let variantResponse;
      try {
        const fetchOptions = await getFetchOptionsWithHeaders(variantUrl, tabId, abortController);
        variantResponse = await fetch(variantUrl, fetchOptions);
        if (!variantResponse.ok) {
          throw new Error(`Failed to fetch variant playlist: HTTP ${variantResponse.status} ${variantResponse.statusText}`);
        }
      } catch (error) {
        if (error.name === 'AbortError' || abortController.signal.aborted) {
          throw error;
        }
        throw new Error(`Network error: Could not fetch video quality variant. ${error.message}`);
      }
      ...
      console.log(`=== VARIANT PLAYLIST CONTENT (${variantText.length} chars, ${variantText.split('\n').length} lines) ===`);
      ...
      if (variantMapLines.length > 0) {
        console.log(`=== FOUND ${variantMapLines.length} MAP/INIT LINES IN VARIANT ===`);
      ...
      if (parsed.initSegmentUrl) {
        ...
        console.warn('No init segment found in selected variant, checking other variants...');
        for (let i = 1; i < Math.min(variantPlaylists.length, 5); i++) { // Check up to 5 variants
          try {
            const otherVariantUrl = fixUrlEncoding(variantPlaylists[i].url);
            const otherFetchOptions = await getFetchOptionsWithHeaders(otherVariantUrl, tabId, abortController);
            const otherResponse = await fetch(otherVariantUrl, otherFetchOptions);
            if (otherResponse.ok) {
              const otherText = await otherResponse.text();
              const otherBaseUrl = otherVariantUrl.substring(0, otherVariantUrl.lastIndexOf('/') + 1);
              const otherParsed = parseM3U8(otherText, otherBaseUrl);
              if (otherParsed.initSegmentUrl) {
                initSegmentUrl = otherParsed.initSegmentUrl;
                console.log(`Found init segment in variant ${i + 1}:`, initSegmentUrl);
                break;
              }
            }
          } catch (err) {
            console.warn(`Failed to check variant ${i + 1} for init segment:`, err.message);
      ...
      console.log(`Playlist length: ${playlistText.length} characters, ${playlistText.split('\n').length} lines`);
      ...
      console.log(`Checking for #EXT-X-MAP in playlist (case-insensitive): ${hasMapTag ? 'FOUND' : 'NOT FOUND'}`);
      ...
      if (allMapLines.length > 0) {
        console.log(`=== FOUND ${allMapLines.length} POTENTIAL MAP/INIT LINES ===`);
        allMapLines.forEach((line, idx) => {
          console.log(`MAP/INIT line ${idx + 1}:`, line);
        });
        console.log('=== END OF MAP/INIT LINES ===');
      } else {
        console.warn('⚠️ No lines containing MAP or INIT found in entire playlist');
      }
      ...
      if (initSegmentUrl) {
        console.log('✅ Found init segment in direct playlist:', initSegmentUrl);
      } else {
        console.warn('⚠️ No init segment found in direct playlist despite parsing');
    ...
    const isMPEGTS = segments.length > 0 && segments.some(seg => seg.includes('.ts') || seg.endsWith('.ts'));
    const isFMP4 = segments.length > 0 && segments.some(seg => seg.includes('.m4s') || seg.includes('frag') || seg.includes('segment'));
    
    if (isMPEGTS) {
      console.log('✅ Detected MPEG-TS playlist (.ts segments) - no init segment needed, segments can be concatenated directly');
    } else if (isFMP4 || initSegmentUrl) {
      console.log('✅ Detected fMP4 playlist - init segment required for QuickTime compatibility');
    } else {
    ...
    if (initSegmentUrl) {
      ...
      const downloadInitSegmentWithRetry = async (retries = 3) => {
        for (let attempt = 0; attempt <= retries; attempt++) {
          ...
          try {
            const fetchOptions = await getFetchOptionsWithHeaders(initSegmentUrl, tabId, abortController);
            ...
            if (initResponse.ok) {
              ...
              const hasFtyp = view[4] === 0x66 && view[5] === 0x74 && 
              ...
              console.log(`Init segment downloaded successfully: ${data.byteLength} bytes (attempt ${attempt + 1}/${retries + 1})`);
              return data;
            } else {
            ...
            if (attempt === retries) {
              // Last attempt failed
              throw new Error(`Failed to download init segment after ${retries + 1} attempts: ${error.message}`);
            }
            
            // Wait before retry (exponential backoff)
            const backoffTime = 1000 * (attempt + 1);
            console.warn(`Retrying init segment download, attempt ${attempt + 2}/${retries + 1}...`);
      ...
      try {
        ...
        console.log('✅ Init segment downloaded and validated - QuickTime compatible');
      } catch (initError) {
        ...
        console.warn(`Failed to download init segment: ${initError.message}. Will attempt workaround with first segment.`);
      ...
      if (isMPEGTS) {
        ...
        console.log('✅ MPEG-TS playlist - no init segment needed, segments will be concatenated directly');
        initSegmentData = null;
      } else {
        // For fMP4, we need to try workaround with first segment
        console.warn('No init segment (map URL) found in playlist. Will attempt workaround with first segment.');
        initSegmentData = null;
      }
    }
    ...
    console.log(`Downloading ${segments.length} segments in batches of ${batchSize}...`);
    
    // Retry function for failed segment downloads (increased retries to 4 = 5 total attempts)
    const downloadSegmentWithRetry = async (segmentUrl, segmentIndex, retries = 4) => {
      // Fix segment URL encoding
      const fixedSegmentUrl = fixUrlEncoding(segmentUrl);
      
      for (let attempt = 0; attempt <= retries; attempt++) {
        // Check if cancelled before each attempt
        if (abortController.signal.aborted || await isDownloadCancelled(downloadId)) {
          throw new DOMException('Download cancelled', 'AbortError');
        }
        
        try {
          const fetchOptions = await getFetchOptionsWithHeaders(fixedSegmentUrl, tabId, abortController);
          ...
          if (error.name === 'AbortError' || abortController.signal.aborted || await isDownloadCancelled(downloadId)) {
            throw new DOMException('Download cancelled', 'AbortError');
          }
          ...
          if (attempt === retries) {
            ...
            if (!errorMsg.includes('503')) {
              console.error(`Failed to download segment ${segmentIndex + 1} after ${retries + 1} attempts:`, errorMessage);
            } else {
              console.warn(`Segment ${segmentIndex + 1} failed with 503 (server overload) after ${retries + 1} attempts - will retry later`);
            }
            return { success: false, index: segmentIndex, url: segmentUrl, error: errorMessage };
          }
          ...
          if (errorMsg.includes('503') || errorMsg.includes('429')) {
            ...
            console.warn(`Rate limited (503/429) for segment ${segmentIndex + 1}, waiting ${Math.round(backoffTime/1000)}s before retry ${attempt + 2}/${retries + 1}...`);
            ...
            console.warn(`Server error for segment ${segmentIndex + 1}, waiting ${Math.round(backoffTime/1000)}s before retry ${attempt + 2}/${retries + 1}...`);
          } else {
            // Other errors - standard exponential backoff with jitter
            backoffTime = 1000 * (attempt + 1) + Math.random() * 500;
            console.warn(`Retrying segment ${segmentIndex + 1}, attempt ${attempt + 2}/${retries + 1}...`);
          }
    ...
    for (let batchStart = 0; batchStart < segments.length; batchStart += batchSize) {
      ...
      console.log(`Processing batch ${batchNumber}/${totalBatches} (segments ${batchStart + 1}-${batchEnd})...`);
      
      await chrome.storage.local.set({
        [`downloadProgress_${downloadId}`]: Math.round((batchStart / segments.length) * 85), // Up to 85% for initial downloading
      ...
      const batchResults = await new Promise(async (resolve, reject) => {
        ...
        checkInterval = setInterval(() => {
          ...
          isDownloadCancelled(downloadId).then((isCancelled) => {
        ...
        try {
          ...
          if (!cancelled && (abortController.signal.aborted || await isDownloadCancelled(downloadId))) {
            reject(new DOMException('Download cancelled', 'AbortError'));
          } else if (!cancelled) {
            resolve(results);
          }
          ...
          if (error.name === 'AbortError' || error.message === 'Download cancelled') {
            reject(error);
          } else if (abortController.signal.aborted || await isDownloadCancelled(downloadId)) {
            reject(new DOMException('Download cancelled', 'AbortError'));
          } else {
            reject(error);
          }
      ...
      for (let i = 0; i < batchResults.length; i++) {
        ...
        if (result.status === 'fulfilled' && result.value.success) {
          ...
          const errorInfo = result.status === 'fulfilled' ? result.value : { index: segmentIndex, url: segmentUrl, error: result.reason?.message || 'Unknown error' };
          failedSegments.push(errorInfo);
          const failedSegmentNum = errorInfo.index !== undefined ? errorInfo.index + 1 : segmentIndex + 1;
          console.warn(`Segment ${failedSegmentNum} failed in batch ${batchNumber}, will retry later`);
        }
      }
    ...
    if (failedSegments.length > 0) {
      ...
      console.log(`Retrying ${failedSegments.length} failed segments with extended retries...`);
      await chrome.storage.local.set({
      ...
      for (let i = 0; i < retryResults.length; i++) {
        ...
        if (result.status === 'fulfilled' && result.value.success) {
          ...
          recoveredCount++;
          console.log(`Successfully recovered segment ${result.value.index + 1}`);
        } else {
          ...
          if (is503) {
            console.warn(`Segment ${failed.index + 1} failed with 503 (server overload) after all retries - will be skipped`);
          } else {
            console.error(`Segment ${failed.index + 1} still failed after all retries:`, failed.error);
          }
        }
      }
      
      console.log(`Recovered ${recoveredCount}/${failedSegments.length} failed segments`);
    ...
    if (segmentData.length === 0) {
      throw new Error('All segments failed to download. Please check your internet connection and try again.');
    }
    ...
    console.log(`Downloaded ${segmentData.length}/${segments.length} segments (${(successRate * 100).toFixed(1)}%)`);
    
    if (missingIndices.length > 0) {
      console.warn(`Missing ${missingIndices.length} segments:`, missingIndices.slice(0, 20).join(', ') + (missingIndices.length > 20 ? '...' : ''));
    }
    ...
    for (const segment of segmentData) {
      ...
      if (segment.index > lastIndex + 1) {
        ...
        console.warn(`Gap detected: missing ${gapSize} segment(s) between index ${lastIndex} and ${segment.index}`);
    ...
    if (orderedSegments.length === 0 || orderedSegments[0].index !== 0) {
      throw new Error('First segment is missing. Video cannot be played without the initial segment.');
    }
    
    // Validate that we have at least the first 5 segments consecutively
    for (let i = 0; i < Math.min(5, segments.length); i++) {
      if (!downloadedIndices.has(i)) {
        throw new Error(`Critical segment ${i + 1} is missing. Video cannot be played without the initial segments.`);
      }
    }
    
    // Create blobs from successful segments in order
    console.log(`Creating blobs from ${orderedSegments.length} segments in correct order...`);
    ...
    for (let i = 0; i < segmentBuffers.length; i++) {
      if (segmentBuffers[i].byteLength === 0) {
        ...
        if (i === 0) {
          console.warn('⚠️ First segment is empty (may have been used as init data)');
          continue;
        }
        console.error(`Segment ${orderedSegments[i].index + 1} is empty!`);
        throw new Error(`Segment ${orderedSegments[i].index + 1} is empty. Video file would be corrupted.`);
      }
    }
    
    // Create blobs in batches to avoid memory issues
    // Filter out empty segments (e.g., first segment if used as init)
    const validSegmentBuffers = segmentBuffers.filter(buf => buf.byteLength > 0);
    ...
    for (let i = 0; i < validSegmentBuffers.length; i += blobBatchSize) {
      const batch = validSegmentBuffers.slice(i, Math.min(i + blobBatchSize, validSegmentBuffers.length));
    ...
    if (!initSegmentData && !isMPEGTS) {
      ...
      console.warn('⚠️ No init segment found in playlist - attempting workaround with first segment...');
      ...
      if (segmentBuffers.length > 0 && segmentBuffers[0]) {
        ...
        if (firstSegment.byteLength >= 8) {
          ...
          const firstHasFtyp = firstView[4] === 0x66 && firstView[5] === 0x74 && 
                               firstView[6] === 0x79 && firstView[7] === 0x70;
          
          if (firstHasFtyp) {
            console.log('✅ First segment contains ftyp box - extracting init data (workaround)');
            ...
            for (let i = 8; i < searchLimit - 8; i++) {
              if (firstView[i] === 0x6D && firstView[i+1] === 0x6F && 
                  firstView[i+2] === 0x6F && firstView[i+3] === 0x76) {
                // Found moov, get its size (4 bytes before)
                if (i >= 4) {
                  const moovSizeBytes = new Uint8Array(firstSegment.slice(i-4, i));
                  const moovSize = (moovSizeBytes[0] << 24) | (moovSizeBytes[1] << 16) | 
                                  (moovSizeBytes[2] << 8) | moovSizeBytes[3];
                  moovStart = i - 4;
                  moovEnd = moovStart + moovSize;
                  console.log(`Found moov atom at offset ${moovStart}, size: ${moovSize} bytes`);
                  break;
                }
              }
            }
            ...
            if (moovStart !== -1 && moovEnd <= firstSegment.byteLength) {
              ...
              console.log(`Extracted init data: ${moovEnd} bytes (ftyp + moov)`);
              ...
              if (moovEnd < firstSegment.byteLength) {
                ...
                for (let i = 0; i < segmentBuffers.length; i += blobBatchSize) {
                  const batch = segmentBuffers.slice(i, Math.min(i + blobBatchSize, segmentBuffers.length));
                ...
                console.warn('⚠️ First segment was entirely init data - segment will be empty but count preserved');
              ...
              console.warn('⚠️ Could not find moov atom, using first 200KB as init data');
            ...
            console.warn('⚠️ First segment does not contain ftyp box - file may not play in QuickTime Player but should work in VLC');
            await chrome.storage.local.set({
              [`downloadStatus_${downloadId}`]: 'Warning: No init segment found. File may not play in QuickTime Player but should work in VLC.'
            });
            // Continue without init segment - some players can handle it
          }
        } else {
          // First segment too small - warn but proceed
          console.warn('⚠️ First segment is too small - file may not play in QuickTime Player but should work in VLC');
          await chrome.storage.local.set({
            [`downloadStatus_${downloadId}`]: 'Warning: No init segment found. File may not play in QuickTime Player but should work in VLC.'
          });
        }
    ...
    if (initSegmentData && !isMPEGTS) {
      ...
      if (initSegmentData.byteLength === 0) {
        throw new Error('Init segment is empty. Video cannot be played without initialization data.');
      }
      
      // Validate init segment has proper MP4 structure
      const initView = new Uint8Array(initSegmentData);
      if (initView.length < 8) {
        throw new Error('Init segment is too small. Video cannot be played without proper initialization data.');
      }
      ...
      const initHasFtyp = initView.length >= 8 && 
                          initView[4] === 0x66 && initView[5] === 0x74 && 
                          initView[6] === 0x79 && initView[7] === 0x70;
      
      if (!initHasFtyp) {
        // Warn but don't fail - file might still work in VLC
        console.warn('⚠️ Init segment does not have valid ftyp box - file may not play in QuickTime Player');
        await chrome.storage.local.set({
          [`downloadStatus_${downloadId}`]: 'Warning: Init segment structure may be invalid. File may not play in QuickTime Player but should work in VLC.'
        });
      } else {
        if (useFirstSegmentAsInit) {
          console.log('✅ Using first segment as init (workaround) - QuickTime compatible');
        } else {
          console.log('✅ Init segment has valid ftyp box - QuickTime compatible');
        }
      }
      ...
      console.warn('⚠️ No init segment available - file may not play in QuickTime Player');
      await chrome.storage.local.set({
        [`downloadStatus_${downloadId}`]: 'Warning: No init segment found. File may not play in QuickTime Player but should work in VLC.'
      });
    } else {
      // MPEG-TS - no init segment needed, this is normal
      console.log('✅ MPEG-TS format - no init segment needed, concatenating segments directly');
    }
    ...
    const mergedBlob = new Blob(finalBlobs, { type: finalMimeType });
    
    // Update filename extension based on format
    // Replace .m3u8 extension with correct video extension
    ...
    console.log(`Final filename: ${finalFilename} (format: ${isMPEGTS ? 'MPEG-TS' : 'fMP4'})`);
    ...
    if (isMPEGTS) {
      ...
      // MPEG-TS packets start with 0x47 every 188 bytes
      const hasSyncByte = mergedArray[0] === 0x47;
      if (hasSyncByte) {
        console.log('✅ Merged file has valid MPEG-TS structure (sync byte 0x47 found) - should play in VLC and most players');
      } else {
        console.warn('⚠️ MPEG-TS file does not start with sync byte 0x47 - may still play but structure might be invalid');
      }
      // MPEG-TS doesn't need ftyp validation
      validatedBlob = new Blob([mergedArray], { type: 'video/mp2t' });
    } else {
      // fMP4 validation: Check if file starts with proper MP4 structure
      // MP4 files should start with a box size (4 bytes) followed by box type
      // ftyp box should be at the beginning (at offset 4)
      const mergedHasFtyp = mergedArray.length >= 8 && 
                            mergedArray[4] === 0x66 && mergedArray[5] === 0x74 && 
                            mergedArray[6] === 0x79 && mergedArray[7] === 0x70;
      
      if (!mergedHasFtyp) {
        const errorMsg = 'Merged file does not have valid MP4 structure (missing ftyp box). File will not play in QuickTime Player.';
        console.error(errorMsg);
        await chrome.storage.local.set({
          [`downloadProgress_${downloadId}`]: 0,
          [`downloadStatus_${downloadId}`]: errorMsg
      ...
      console.log('✅ Merged file has valid MP4 structure - should play in QuickTime and VLC');
    ...
    if (missingIndices.length > 0) {
      console.warn(`⚠️ HLS segments merged with ${missingIndices.length} missing segments - file may have playback issues. Prefer MP4 downloads when available.`);
    } else {
      console.log('✅ All segments downloaded successfully - video should play correctly in QuickTime and VLC');
    }
    
    // Set progress to 100% BEFORE downloadBlob so polling sees completion immediately
    // This ensures notification shows "complete" even if downloadCompleted message is delayed/lost
    await chrome.storage.local.set({
    ...
    setTimeout(() => {
      chrome.storage.local.remove([
        ...
        if (chrome.runtime.lastError) {
          ...
          console.log('Cleaned up all download storage from downloadM3U8:', downloadId);
        }
      });
    }, 15000);
      ...
      setTimeout(() => {
        chrome.storage.local.remove([
          ...
          if (chrome.runtime.lastError) {
            ...
            console.log('Cleaned up all download storage from downloadM3U8 (cancelled):', downloadId);
          }
        });
      }, 2000);
    ...
    if (error.message) {
      if (error.message.includes('Failed to fetch') || error.message.includes('NetworkError')) {
        errorMessage = 'Network error: Failed to download video segments. Please check your internet connection and try again.';
      } else if (error.message.includes('No segments found')) {
        errorMessage = 'No video segments found in playlist. The video may not be available for download.';
        ...
        errorMessage = 'Failed to download video segments. The video may be protected or unavailable.';
      } else {
    ...
    setTimeout(() => {
      chrome.storage.local.remove([
        ...
        if (chrome.runtime.lastError) {
          ...
          console.log('Cleaned up all download storage from downloadM3U8 (error):', downloadId);
        }
      });
    }, 10000);
